...
| 触摸检测/切页 | EXTI 唤醒 + 主循环任务 | 事件触发 + 50ms 兜底 | 输入事件 + 轻量状态观测 |
...
| `pollTouch()` | 50ms 兜底 | 观测触摸按下/释放状态，补偿漏中断场景 |
...
触摸不建议把坐标读取放进中断。当前实施采用 `TOUCH_PEN` 上升沿 EXTI 只置标志，把“切页”解释为触摸释放事件；主循环下一个 tick 再消费该事件。

| 阶段 | 做法 | 说明 |
| :--- | :--- | :--- |
| 当前阶段 | `TOUCH_PEN` EXTI 上升沿置位 | 中断只表示一次触摸释放事件发生 |
| 当前阶段 | 主循环下一个 tick 消费触摸事件 | 切页动作始终留在非 ISR 环境执行 |
| 兜底机制 | 50ms 周期检查 `isTouched()` | 防止漏边沿时丢失一次按下/释放周期 |
...
5. `HAL_GPIO_EXTI_Callback(TOUCH_PEN_Pin)` 只置触摸待处理标志
6. 任意一次有效触摸释放都可切页，不再限制右半屏
7. 按键、触摸、LED、LCD、AHT20/BMP280 和报警逻辑上板无回归
